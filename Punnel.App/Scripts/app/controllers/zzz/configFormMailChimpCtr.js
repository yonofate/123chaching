angular.module("punnelApp").controller("configFormMailChimpCtr", ["$rootScope", "$state", "$scope", "$translate", "$stateParams", "$timeout", "$restful", function (a, b, c, d, e, f, g) {
    c.nameMailC = "", c.apiKeyMailC = "", c.listConfig = [], c.type = "listAccount", c.listCampant = [], c.campainSl = {}, c.accountIdMail = "", c.edit = "false", c.model = {
        ...
        $("#configFormMailChimp").modal("hide"), $("#saveDataForms").modal("show"), c.type = "listAccount"
        ...
        if (c.accountIdMail && c.accountIdMail.length > 0 && c.campainSl && c.campainSl.id && c.campainSl.id.length > 0) {
            ...
            a.configFormServe || (a.configFormServe = []), a.configFormServe.push(b), $("#configFormMailChimp").modal("hide"), $("#saveDataForms").modal("show"), c.type = "listAccount", c.campainSl = {}
        } else PN_PAGE.messageLadi("Lựa chọn tài khoản và chiến dịch của bạn để tiếp tục!")
    }, c.getTemplate = function (a) {
        return a.id === c.model.selected.id ? "lpeditMailC" : "lpdisplayMailC"
