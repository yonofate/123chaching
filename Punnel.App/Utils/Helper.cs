...
namespace Punnel.App
{
    public class Helper
    {
        ...
        public static int GetLevel(string userId)
        {
            int lv = -1;
            bool k = false;
            var claimLevel = new AuthRepository().GetClaim(userId, "Level");
            if (claimLevel != null)
            {
                k = int.TryParse(claimLevel.Value, out lv);
            }
            if (k == true && lv >= 0) return lv;
            else
            {
                IUow uow = Core.Entities.ObjectFactory.GetInstance<IUow>();
                var profile = uow.UserProfile.Get(userId);
                new AuthRepository().UpdateClaim(userId, new Claim("Level", profile.Level.ToString()));
