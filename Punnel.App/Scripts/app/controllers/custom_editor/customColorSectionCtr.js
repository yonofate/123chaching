angular.module("punnelApp").controller("customColorSectionCtr", ["$rootScope", "$state", "$scope", "$translate", "$stateParams", function (a, b, c, d, e) {
    ...
    c.colorSelect = "#ffffff", c.colorTmpSelect = f.hexToRgba(colorUsing, 1), c.itemsGr = [], c.getItemGroupColor = function (a) {
        ...
        void 0 != b && "" != b ? (c.colorTmpSelect = b, $("input.minicolor").colorpicker("setValue", c.colorTmpSelect)) : void 0 == a ? c.colorTmpSelect = $("input.minicolor").val() : c.colorTmpSelect = $(a.target).val(), c.colorSelect = c.colorTmpSelect
    }, c.close = function () {
        if (void 0 != selectedItem && selectedItem.length > 0) {
            var a = new OptionWiget;
            a.addElementUndo("", selectedItem)
        }
        if (savedSel = "", fadeOutAnimate($(".ngdialog.custom_color_section")), "shape" == selectedItem.attr("pn-type")) $('.settings[pn-setting="shape"]').addClass("active");
        else {
