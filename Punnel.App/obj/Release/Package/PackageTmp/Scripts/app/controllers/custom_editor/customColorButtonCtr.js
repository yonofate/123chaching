angular.module("punnelApp").controller("customColorButtonCtr", ["$rootScope", "$state", "$scope", "$translate", "$stateParams", function (a, b, c, d, e) {
    ...
    c.colorSelect = "#ffffff", c.colorTmpSelect = f.hexToRgba(colorUsing, 1), c.itemsGr = [], c.getItemGroupColor = function (a) {
        ...
        if (savedSel = "", fadeOutAnimate($(".ngdialog.custom_color_button")), "shape" == selectedItem.attr("pn-type")) $('.settings[pn-setting="shape"]').addClass("active");
        else {
